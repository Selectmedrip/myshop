<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Макс Дев</title>
    <link href="bootstrap/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="./css/blocks.css">
    <link href="./css/style.css" rel="stylesheet">
</head>
<body>
    <?php 
    include("header.php");
    ?>

    <div class="container pb-4 pt-4">
        <div class="align-items-center row">
                <div class="col">
                    <hr class="mb-0 mt-0"> 
                </div>
                <div class="col-auto">
                    <h2 class="fw-normal lead mb-0 text-dark">О нас</h2> 
                </div>
                <div class="col">
                    <hr class="mb-0 mt-0"> 
                </div>
        </div>
    </div>

    <section class="bg-light mb-4 mt-4 pb-5 pt-5 text-center"> 
                <div class="container pb-3 position-relative pt-3"> 
                    <div class="row"> 
                        <div class="col-lg-8 ms-auto me-auto"> 
                            <div id="carousel5" class="carousel slide" data-bs-ride="carousel"> 
                                <div class="carousel-inner"> 
                                    <div class="carousel-item active"> 
                                        <img src="https://opis-cdn.tinkoffjournal.ru/mercury/neural-photo-1.iehmu4..jpeg" class="img-fluid mb-3 rounded-circle testimonial_img" width="120" height="120" alt="User image">
                                        <p class="mb-4">    Пусть он определенным образом убежит из двух домов. Есть такие, которые рожают двоих мужчин и на короткое время от боли размягчают волосы. Элитарные, оставляют ли они на время свои увлечения, или оставляют их ради удовольствия, если только не оставят.</p>
                                        <h2 class="fw-bold h5 mb-0 text-dark">Юлия Крючкова</h2>
                                        <p class="small">Генеральный директор</p> 
                                    </div>
                                    <div class="carousel-item"> 
                                        <img src="https://photoplay.ru/sites/default/files/styles/r-1-1-mobile/public/gallery_images/bez_nazvaniya_4.jpg?itok=BkR2aUAX" class="img-fluid mb-3 rounded-circle testimonial_img" width="120" height="120" alt="User image">
                                        <p class="mb-4">Пусть он определенным образом убежит из двух домов. Есть такие, которые рожают двоих мужчин и на короткое время от боли размягчают волосы. Элитарные, оставляют ли они на время свои увлечения, или оставляют их ради удовольствия, если только не оставят.</p>
                                        <h2 class="fw-bold h5 mb-0 text-dark">Максим Шастунов</h2>
                                        <p class="small">Генеральный менеджер</p> 
                                    </div>                                     
                                </div>
                                <ol class="carousel-indicators position-relative"> 
                                    <li data-bs-target="#carousel5" data-bs-slide-to="0" class="bg-dark  active" aria-current="true"></li>                                                   
                                    <li data-bs-target="#carousel5" data-bs-slide-to="1" class="bg-dark"></li>                                     
                                </ol>                                 
                            </div>                             
                        </div>                         
                    </div>                     
                </div>                 
            </section>
            <section class="pb-5 pt-5 text-center">
                <div class="container"> 
                    <div class="justify-content-center row"> 
                        <div class="col-lg-4 col-sm-6 pb-3 pt-3">
                            <div>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="3rem" height="3rem" class="mb-3 text-dark">
                                    <g>
                                        <path fill="none" d="M0 0h24v24H0z"/>
                                        <path d="M1.946 9.315c-.522-.174-.527-.455.01-.634l19.087-6.362c.529-.176.832.12.684.638l-5.454 19.086c-.15.529-.455.547-.679.045L12 14l6-8-8 6-8.054-2.685z"/>
                                    </g>
                                </svg>
                                <h2 class="fw-bold h5 mb-3 text-dark">24/7 Онлайн поддержка</h2>
                                <p class="mb-0">Наши менеджеры всегда на связи в любое время года.<br>Узнать статус заказа можно через бота в Telegram.</p>
                            </div>                             
                        </div>
                        <div class="col-lg-4 col-sm-6 pb-3 pt-3">
                            <div>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="3rem" height="3rem" class="mb-3 text-dark">
                                    <g>
                                        <path fill="none" d="M0 0h24v24H0z"/>
                                        <path d="M3.783 2.826L12 1l8.217 1.826a1 1 0 0 1 .783.976v9.987a6 6 0 0 1-2.672 4.992L12 23l-6.328-4.219A6 6 0 0 1 3 13.79V3.802a1 1 0 0 1 .783-.976zM12 13.5l2.939 1.545-.561-3.272 2.377-2.318-3.286-.478L12 6l-1.47 2.977-3.285.478 2.377 2.318-.56 3.272L12 13.5z"/>
                                    </g>
                                </svg>
                                <h2 class="fw-bold h5 mb-3 text-dark">Гарантия 100%</h2>
                                <p class="mb-0">Сертификат о надлежащем качестве от самого производителя <br>Lego</p>
                            </div>                             
                        </div>
                        <div class="col-lg-4 col-sm-6 pb-3 pt-3">
                            <div>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="3rem" height="3rem" class="mb-3 text-dark">
                                    <g>
                                        <path fill="none" d="M0 0h24v24H0z"/>
                                        <path d="M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12 6.477 2 12 2zM7.197 14.682l-2.175 2.174a8.549 8.549 0 0 0 1.818 1.899l.305.223 2.173-2.175a5.527 5.527 0 0 1-1.98-1.883l-.14-.238zm9.606 0a5.527 5.527 0 0 1-1.883 1.98l-.238.14 2.174 2.176a8.549 8.549 0 0 0 1.899-1.818l.223-.304-2.175-2.174zM12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8zM7.145 5.022a8.549 8.549 0 0 0-1.9 1.818l-.223.305 2.175 2.173a5.527 5.527 0 0 1 1.883-1.98l.238-.14-2.173-2.176zm9.71 0l-2.173 2.175a5.527 5.527 0 0 1 1.98 1.883l.14.238 2.176-2.173a8.549 8.549 0 0 0-1.818-1.9l-.304-.223z"/>
                                    </g>
                                </svg>
                                <h2 class="fw-bold h5 mb-3 text-dark">Бесплатная доставка</h2>
                                <p class="mb-0">Доставка по всему миру, в любую точку планеты.<br>Для недружественных стран есть комиссия на доставку в виде 5% налога</p>
                            </div>                             
                        </div>                         
                    </div>                     
                </div>
            </section>

        <div class="container mt-5 pb-4 pt-4"><hr class="mb-0 mt-0"></div>
    <?php
      include("footer.php");
    ?>

        <script src="bootstrap/js/bootstrap.min.js"></script>
</body>
</html>